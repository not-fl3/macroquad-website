...
fn take_screenshot(name: &str) {
	Command::new("cargo")
		...
		.output()
		.unwrap();
	std::fs::copy(format!("{}/screenshot.png", MACROQUAD_PATH), format!("docs/images/{}.png", name)).unwrap();
}

fn build_wasm(name: &str) {
	Command::new("cargo")
		.args(&["build", "--example", name, "--release", "--target", "wasm32-unknown-unknown"])
		.current_dir(MACROQUAD_PATH)
		.output()
		.unwrap();
    let new_name = format!("docs/wasms/{}.wasm", name);
	std::fs::copy(format!("{}/target/wasm32-unknown-unknown/release/examples/{}.wasm", MACROQUAD_PATH, name), &new_name).unwrap();
...
fn get_examples() -> Vec<String> {
	std::fs::read_dir(format!("{}/examples/", MACROQUAD_PATH))
    	...
    	.map(|mut name| { name.truncate(name.len() - RUST_EXTENSION.len()); name })
    	.collect::<Vec<_>>()
}
...
fn copy_other_files() {
    ...
    fs_extra::copy_items(&other_files, "docs/examples", &fs_extra::dir::CopyOptions {
    	skip_exist: true,
...
fn copy_assets() {
    ...
    fs_extra::copy_items(&assets, "docs", &fs_extra::dir::CopyOptions {
        skip_exist: true,
        ..fs_extra::dir::CopyOptions::new()
    }).unwrap();
}
